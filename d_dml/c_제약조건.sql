...
/*
    ...
    - null 허용 가능 (비워둘 수 있음) - PK 차이점
    - 한 테이블에 여러 개 지정 가능 - PK 차이점
    EX) 아이디, 이메일, 주민번호 등에 사용

*/

# b_제약 조건 - DB(스키마)생성으로 서버에 저장 중
# Java파일의 저장 개념과 다름(파일 저장)
# DB 생성 유무 필요 시 새 파일 생성 후 USE 사용으로 서버 확인 가능
use `example`;
...
-- insert into `users`
-- values
-- 	(2, 'ewq234', 'wer123323', 'qwe123@example.com', '654321');
...
insert into `employees`
values
	(5, '안미향', null); -- check 제약 조건은 null 여부 확인
...
values
	(1, '권민현', '010-1111-2222'),
	(2, '김세훈', '010-2222-3333');
	-- (2,'김세훈', null); 'phone' cannot be null	0.000 sec
