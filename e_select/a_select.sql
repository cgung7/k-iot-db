...
# - 공백 사용 시 따옴표로 반드시 지정
select
	`member_id` as 고유번호, `name` as '회원이름', `contact` as '회원 연락처'
from
	`members`;
...
# 3) null값 연산
# : 직접적인 연산 X (일치와 불일치 값 조건 검사 불가)
-- select * from `members`
-- where
-- 		points = null;

# cf) null값이 '값이 없음'을 나타냄: 그 어떤 값과도 비교하거나 연산이 불가능
#	>> is null, is not null 연산자 사용 - null 여부 확인 가능(true / falde)

# A is null
# null인 경우에는 true, 아닌경우 false

# A is not null
# null인 경우 false, 아닌경우 true

select * from `members`
where
	points is not null;
    
# 4) betweem A and B 연산자
# : A와 B사이에
...
# 5) in 연산자
# : 지정할 범위가 문자 데이터를 나열
# - 지정됭 리스트 중애서 일치하는 값이 있으면 true
select * from `members`
where
	area_code in('Soeul', 'Busan', 'jeju');
    # >> 문자열 데이터에 대한 or식 간소회
    ...
    #	(이름이 두 번째 글자가 u인 모드 회원 조회): _u%
    select * from `members`
    where
		name like '_u%';
        
	# 이름이 네글자인 모든 회원 조회____
    select * from `members`
    where
		name like "____";
