...
# > select ~ from ~ where 구문을 전체 복습

USE `korea_db`;

# 1) 이름(name)을 기준으로 정렬
select * from `members`
order by name ; -- 오름 차순 asc 생략 가능

# 2) 3번째 레코드 부터 2개의 행 조회
...
# 4) 서로 다른 등급 목록 조회 (중복되는 등급 생략 - distinct 사용)
...
-- 집계 함수: avg(평균), sum(합), max(큰값), min(작은값), count(수)
