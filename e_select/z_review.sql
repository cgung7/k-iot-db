...
# : 데이터베이스의 구조 정의
# - CREATE(생성), ALTER(수정), DROP(삭제), TRUNCATE(기존 구조 유지, 데이터 만 삭제)

# 2) DML(Data Manipulation Lanuage)
# : 데이터 조작(CRUD)
...
-- +) name과 area_code 컬럼은 각각 '회원 이름', '지역 코드'로 컬럼명 조회 // WHERE 와 AND LIKE로 이름명 확인
SELECT 
