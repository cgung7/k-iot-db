...
# 2) 속성
-- 엔티티의 세부정보, 설명 항목
-- 일반 속성(이름, 주소, 전화번호 등), 기본키(PK: 회원 ID, 학번 등), 외래키(FK: 주문 테이블의 고객 ID)
-- 타원형 (속성명은 원 안에 명시)
...
### ERD 표현 규칙 ###
# 1) 부모 PK를 자식이 FK로 가짐
# 	> 실선 관계:자식이 부모의 존재에 존속됨 (부모가 있어야만 자식이 존재 - PF + FK)
#	> 점선 관계: 자식이 독립적은 존재 가능 (부모 없이도 자식 생성 가능 - FK)

# 2) 예시 1
# 학생과 수강 내역
# 학생 테이블) 부모 테이블 - 학번 PK, 학생명 ... 등
# 수강 내역 티이블) 자식 테이블 - 주문 ID, PK,학번 FK, 강의명 ... 등
...
# >> 관계: 자식 테이블의 기본키가 부모의 기본키를 포함하거나 그대로 사용 (1:1관계 - 실선)

# 4) 예시 3
# 회원과 즐겨찾기 프로그램
# 회원 테이블) 부모 - 회원 ID PK, 이름 ... 등
# 즐겨찾기 프로그램 테이블) 고유 번호 ID PK, 회원번호 FK, 프로그램명 등
# >> 비회원 또는 임시 사용자도 '즐겨찾기 기능' 에 대해 적용
# 		- 회원없이도 자식 테이블이 존재 가능한 구조로 설계가능(1:N 관계)

# cf) N:M 관계 처리 방법 (다대다)
# : 관계형 데이터 베이스(RDBMS)는 직접적인 N:M 관계를 허용하지 않음
# : 중간 테이블을 시용하여 분해
# EX) 학생 - 과목
#		1명의 학생은 여러 과목을 수강
#		1개의 과목은 여러 학생이 수강alter
# >> 중간 테이블: 수강(student_subject)
#		- 학생 ID (FK)
#		- 과목 ID (FK)
#		>> 연결되는 두 개의 참조값을 복합키 설명

#		cf) 복합키: 두 개 이상의 컬럼으로 구성된 기본카
#		- 수강날짜 등 초기 속성 정의 가능
...
create table course_enrollment (
    ...
    # >> student_id와 course_id 의 조합이 비워질 수 없으며, 둘 다 합쳐서 유일한 값이 되어야 함
