...
# 2. DML (Manipulation): 데이터 조작언어 (데이터 DRUD)
...
### DCL 사용 목적 ###
# 1. 보안 강화: 민감한 데이터에 접근 가능한 서용자 제한 가능
...
# '계정명'@'접속범위설정' INDENTIFIED BY '사용자_로그인_비밀번호_설정';

# cf) 접속 범위 설정
# - 'localhost': 현 시스템 컴퓨터 (MySQL 서버가 설치된 이 커퓨터에서만 로그인 가능)
