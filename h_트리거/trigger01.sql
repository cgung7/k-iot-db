...
### 트리거(trigger) ###
# '방아쇠', 자동 실행
# : 테이블의 특정 이벤트가 발생할 때 자동으로 실행되는 저장된 프로시저 형태의 객체
...
# - 재고 수량 자동 조정, 포인트 자동 정립 등 '자동화 된 비즈니스 로직 처리'
...
/*
    ...
    create trigger 트리거명
		트리거시점(종류: after, before) 이벤트 종류(insert, uodate, delete)
		on 테이블명
		for each row
